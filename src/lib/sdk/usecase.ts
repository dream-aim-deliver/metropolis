...
> extends BaseUseCase<TRequestModel, TResponseModel, TErrorModel> {
  ...
  processGatewayResponse(response: TDTO): TResponseModel | TErrorModel {
    ...
    const commonError: BaseErrorResponseModel | undefined =
      handleCommonGatewayErrors<TDTO>(response)
    if (commonError) {
...
> extends BaseSingleEndpointUseCase<
  ...
  TDTO
> {
  protected postProcessingPipelineElements: BasePostProcessingPipelineElement<
    TRequestModel,
    TResponseModel,
    TErrorModel,
    any
  ...
  constructor(
    ...
    postProcessingPipelineElements: BasePostProcessingPipelineElement<
      TRequestModel,
      TResponseModel,
      TErrorModel,
      any
  ...
  async execute(requestModel: TRequestModel): Promise<void> {
    ...
    for (let i = 0; i < this.postProcessingPipelineElements.length; i++) {
      ...
      const response: TResponseModel | TErrorModel = await element.execute(
        requestModel,
    ...
    const { isValid, errorModel } =
      this.validateFinalResponseModel(currentResponseModel)
    if (!isValid) {
  ...
  implements BaseStreamableInputPort<AuthenticatedRequestModel<TRequestModel>>
{
  ...
  constructor(
    presenter: BaseStreamingOutputPort<
      TResponseModel,
      TErrorModel,
      TStreamViewModel
    >,
    ...
    >
{
  ...
  constructor(
    ...
    this.finalResponseValidationTransform =
      new BaseFinalResponseModelValidatorPipelineElement<
        TResponseModel,
        TErrorModel
   ...
   * @param callback
   */
  _transform(chunk: any, encoding: any, callback: TransformCallback): void {
 ...
 */
export function handleCommonGatewayErrors<TDTO extends BaseDTO>(
  error: TDTO,
