...
import { TRequestModel } from './usecase-models'
/**
...
> {
  ...
  processGatewayResponse(
    responseModel: TResponseModel,
    dto: TDTO,
  ): TResponseModel | TErrorModel {
    if (dto.status === 'success') {
      return responseModel
    }
    const commonError: BaseErrorResponseModel | undefined =
      handleCommonGatewayErrors<TDTO>(dto)
    if (commonError) {
   ...
   */
  abstract transformResponseModel(
    responseModel: TResponseModel,
    dto: TDTO,
  ): TResponseModel | TErrorModel
  ...
  async execute(
    requestModel: TRequestModel,
    responseModel: TResponseModel | TErrorModel,
  ): Promise<TResponseModel | TErrorModel> {
    ...
    const response: TResponseModel | TErrorModel = this.processGatewayResponse(
      responseModel,
      dto,
    )
    ...
    const transformedResponseModel = this.transformResponseModel(
      responseModel,
      dto,
    )
...
> extends Transform {
  ...
  async _transform(
    ...
    try {
      responseModel = responseModel as TResponseModel
      const dto = await this.makeGatewayRequest(requestModel, responseModel)
      const response: TResponseModel | TErrorModel =
        this.processGatewayResponse(responseModel, dto)
      callback(null, {
