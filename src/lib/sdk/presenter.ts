...
export abstract class BaseStreamingPresenter<
  ...
  extends Transform
  implements
    BaseStreamingOutputPort<TResponseModel, TErrorModel, TStreamViewModel>
{
  ...
  _transform(
    ...
    try {
      ...
      if (responseModel.status === 'success') {
        const viewModel = this.streamResponseModelToViewModel(responseModel)
        callback(null, JSON.stringify(viewModel) + '\n')
      } else if (responseModel.status === 'error') {
        const viewModel = this.streamErrorModelToViewModel(
          responseModel as unknown as TErrorModel,
        )
        callback(null, JSON.stringify(viewModel) + '\n')
      } else {
        callback(null)
