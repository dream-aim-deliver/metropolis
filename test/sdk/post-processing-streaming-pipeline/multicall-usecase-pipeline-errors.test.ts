...
describe('Post Processing Streaming Pipeline Error Handling', () => {
  class StupidPipelineElement extends BaseStreamingPostProcessingPipelineElement<TRequestModel, TResponseModel, BaseErrorResponseModel, TDTO> {
    makeGatewayRequest(requestModel: { authToken: string }, responseModel: TResponseModel): Promise<TDTO> {
  ...
  > {
    ...
    validateRequestModel(requestModel: { authToken: string }): BaseErrorResponseModel | undefined {
      return undefined
    }
    makeGatewayRequest(requestModel: { authToken: string }): Promise<BaseStreamableDTO> {
